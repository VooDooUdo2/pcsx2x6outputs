...
#pragma pack(pop)
	...
	struct GunCon2State
	{
		...
		~GunCon2State();
		void SetTriggerState(bool pressed);
		...
		: port(port_)
	{
		...
		Console.WriteLn("RECOIL : GunCon2State -> Start GunCon %d for %s", port, serial);

		if (active_game == "" && serial != "")
		{
			active_game = serial;

			if (active_game == "NM00021" || active_game == "NM00003") 
			{
	...
	void GunCon2State::SetTriggerState(bool pressed)
	{
		triggerIsActive = pressed;
	}

	void GunCon2State::threadOutputs()
	{
		Console.WriteLn("RECOIL : Recoil Thread Start %s", active_game);
		...
		while (active_game != "")
		{
			...
			if (active_game == "NM00003") // Vampire Night
			{
				bool valid_query = false;
				u32 ammoCount = 0;
				if (port == 0)
				{
					valid_query = true;
					ammoCount = memRead32(0x3CBBC4);
				}
				if (port == 1)
				{
					valid_query = true;
					ammoCount = memRead32(0x3CBE78);
				}
				if (valid_query)
				{
					if (ammoCount < lastAmmo && triggerIsActive)
			...
			if (output_signal != "")
			{
				Console.WriteLn("RECOIL: Recoil thread sending gunshot signal for port %d", port);
	...
	void GunCon2Device::SetBindingValue(USBDevice* dev, u32 bind_index, float value) const
	{
		...
		if (bind_index < BID_RELATIVE_LEFT)
		{
			...
			if (ACJV::enabled)
			{
				...
				case BID_TRIGGER:
				{
					s->SetTriggerState(pressed); // Update trigger state for recoil thread, it's not picky about which player is which
